...
const EventPreview = ({ event, type }) => {
  ...
  const dynamicTitle =
    type === "large"
      ? "big-title"
      : type === "popular" || type === "main-event-content"
      ? "popular-title"
      : "post-title";
  const textClass =
    type === "news" ? "post-preview-text" : "news-preview-container";
  // data-----------
  ...
  const eventPreview =
    type == "post" || type == "news"
      ? "dark-post-preview-container"
      : "post-preview-container";
  const eventPreviewTitle =
    type == "post" || type == "news"
      ? "dark-post-preview-title"
      : "post-preview-title";
  const sliceOfDescription =
    type == "popular" ? "light-slice-of-description" : "slice-of-description";
  ...
  return (
    <div className={eventPreview}>
      ...
      {type == "square" && <div className="time">{year}</div>}
      <div className={textClass}>
        <div className={eventPreviewTitle}>
          {
            <NavLink to={`/events/${event?.id}`}>
              <h3 className={dynamicTitle}>{event?.title}</h3>
            </NavLink>
          }
        </div>
